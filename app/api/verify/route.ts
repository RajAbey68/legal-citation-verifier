...
export async function POST(req: NextRequest) {
  const { claim, sources = ['sra', 'fca', 'eu', 'lawgazette', 'legal_regulation', 'ncsc', 'gov_uk', 'bsi'] } = await req.json();
  if (!claim) return NextResponse.json({ error: 'claim is required' }, { status: 400 });

  // Always include protocol docs alongside selected sources
  const sourceIds = [
    ...(sources as string[]).flatMap((g) => SOURCE_IDS[g] ?? []),
    ...SOURCE_IDS._protocol,
  ];
  ...
  const result = await client.callTool({
    ...
    arguments: {
      ...
      query: `You are a legal accuracy gatekeeper for The Digital Law Firm book (UK). Apply these rules strictly:
...
Claim: "${claim}"`,
  ...
  return NextResponse.json({ verdict, answer, claim });
}

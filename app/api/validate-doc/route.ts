...
async function extractText(file: File): Promise<string> {
  ...
  if (ext === '.gdoc' || ext === '.gsheet' || ext === '.gslides') {
    return NextResponse.json as never; // handled separately below
  }
...
export async function POST(req: NextRequest) {
  ...
  const sources: string[] = sourcesRaw ? JSON.parse(sourcesRaw) : ['sra', 'fca', 'eu', 'lawgazette', 'ico', 'lawsociety', 'iso', 'market', 'legal_regulation', 'ncsc', 'gov_uk', 'bsi'];
  ...
  const sourceIds = sources.flatMap((g) => SOURCE_IDS[g] ?? []);

  const query = `You are a legal accuracy validator. Below is a document submitted for validation.

Your job is to check the factual and regulatory claims in this document against the authoritative UK legal sources in this notebook (SRA, FCA, EU AI Act, Law Gazette).
